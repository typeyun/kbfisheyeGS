...
files = os.listdir(args.source_path + "/sparse")#列出稀疏重建结果中的文件，并创建一个新目录以存放这些文件。
os.makedirs(args.source_path + "/sparse/0", exist_ok=True)
# Copy each file from the source directory to the destination directory
for file in files:#将所有文件移动到新创建的 sparse/0 目录中，排除名为 '0' 的文件。
    if file == '0':
        continue
