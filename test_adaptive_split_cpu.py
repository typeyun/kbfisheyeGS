...
def test_hessian_approximation():
    """测试Hessian范数近似计算"""
    print("="*60)
    ...
    # 当前梯度
    current_grad = torch.randn(num_points, 3)
    
    # 历史梯度
    history_grad = torch.randn(num_points, 3)
...
def test_adaptive_threshold():
    ...
    # 模拟不同曲率的区域
    # 高曲率区域：大的Hessian范数
    high_curvature = torch.ones(30, 1) * 3.0
    # 低曲率区域：小的Hessian范数
    low_curvature = torch.ones(30, 1) * 1.0
    # 中等曲率
    medium_curvature = torch.ones(40, 1) * 1.5
...
def test_split_logic():
    ...
    # 模拟梯度
    grads = torch.rand(num_points) * 0.001
    
    # 模拟Hessian范数（不同区域不同）
    hessian_norm = torch.ones(num_points)
